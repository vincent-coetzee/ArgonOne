...
internal class SingleValueContainer {
    ...
    init(from buffer: UnsafeRawBufferPointer) {
        let tmp = UnsafeMutableRawBufferPointer.allocate(byteCount: buffer.count,alignment: 1)
    ...
    init<T: EncodableType>(_ value: T) {
        ...
        let buffer = UnsafeMutableRawBufferPointer.allocate(byteCount: Offset.value + MemoryLayout<T>.stride,alignment:MemoryLayout<T>.alignment)
    ...
    init(_ value: String) {
        ...
        let buffer = UnsafeMutableRawBufferPointer.allocate(byteCount: Offset.value + (utf8.count * MemoryLayout<Unicode.UTF8.CodeUnit>.stride),alignment: MemoryLayout<UTF8.CodeUnit>.alignment)
